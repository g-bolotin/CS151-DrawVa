...
public class DrawVaController extends JComponent implements Observer {
    ...
    public DrawVaController(DrawVaModel model) {
        this.model = model;
        this.addMouseListener(new MouseAdapter() {
            public void mousePressed(MouseEvent e) {
                ...
                if (model.getDrawMode()) {
                    if (model.getDrawingMode() == DrawVaModel.drawingModeType.FREEFORM) {
                        ArrayList<Point> freeHandPoints = new ArrayList<>();
                        freeHandPoints.add(model.getClickBegin());
                        DrawVaModel.CanvasShape cs = new DrawVaModel.CanvasShape(
                                freeHandPoints,
                    ...
                    while(it.hasPrevious()) {
                    	DrawVaModel.CanvasShape cs = it.previous();
    ...
    private void drawSelectRectangle(DrawVaModel.CanvasShape cs, Graphics2D g2) {
    ...
    private AffineTransform generateAffine(DrawVaModel.CanvasShape cs, AffineTransform affine) {
    ...
    private boolean hitTest(DrawVaModel.CanvasShape cs, Point mouse, int threshold) {
    ...
    private boolean checkTranslated(DrawVaModel.CanvasShape cs) {
        return cs.rotate != 0 ||
                cs.scaleX != 1 ||
                cs.scaleY != 1 ||
                cs.translateX != 0 ||
